...
- **Colours the answer amber, red or green** — after 1 wrong answer in a unit, the text they're typing is checked against the correct answer, live, before they press Enter: green when it's right, amber when it's wrong. From the second wrong answer on, a wrong entry turns red instead of amber.
- **Makes a wrong answer take two Enters** — while that hint is live, the first Enter on a wrong answer is swallowed and the field shakes, so a hurried press doesn't spend the attempt before they've looked at the colour. A correct answer still submits on the first press, as does anything the script can't check.
...
Hiding the timer, the answer hints (the colour and the double Enter together), and the play-again button are each a checkbox in the extension's toolbar popup in the old Chrome-extension version of this project; here they're Tampermonkey menu commands (see below).
...
- ✅/⬜ Hide the timer
- ✅/⬜ Amber after 1 wrong, red after 2, double Enter to submit
